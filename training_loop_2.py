...
GENERATOR_OUTPUT = "output/model_2_gen_output/"
DISCRIMINATOR_OUTPUT = "output/model_2_disc_output/"
CHECKPOINT_DIR = "output/model_2_training_ckeckpoints/"
LOG_DIR = "output/model_2_logs/"
...
def save_decoder_img(model, img, epoch,direct):
    [pred,part1,part2]  = model(img,label='real', part=1)
    predictions = np.clip(deprocess(part2), 0, 255).astype(np.uint8) 
...
class FastGAN(tf.keras.Model):
    ...
    def reconstruction_loss(self,decoded_big,decoded_part, part, real_img):
    ...
    def train_step(self, real_img):
        ...
        with tf.GradientTape() as discriminator_tape:
            ...
            [real_discriminator_logits,decoded,decoded_part] = self.discriminator(real_augmented, training=True,label='real',part=part )
            ...
            reconstruction_loss = self.reconstruction_loss(decoded,decoded_part,part, real_img)
