...
GENERATOR_OUTPUT = "output/model_3_gen_output/"
DISCRIMINATOR_OUTPUT = "output/model_3_disc_output/"
CHECKPOINT_DIR = "output/model_3_training_ckeckpoints/"
LOG_DIR = "output/model_3_logs/"
...
def save_decoder_img(model, img, epoch,direct):
    [pred,part1,part2,part3]  = model(img,label='real', part=1)
    predictions = np.clip(deprocess(part3), 0, 255).astype(np.uint8) 
...
def get_img_center(img):
    hw = img.shape[2]//2
    return img[:, hw-64:hw+64, hw-64:hw+64, :]
...
class FastGAN(tf.keras.Model):
    ...
    def reconstruction_loss(self,decoded_big,decoded_part, part,center, real_img):
        real_img = deprocess(real_img)
        big = deprocess(decoded_big)
        decoded_part = deprocess(decoded_part)
        center = deprocess(center)

        scenery_real = tf.image.resize(real_img,[128,128])
        part_real = tf.image.resize(crop_image_by_part(real_img, part),[128,128])
        center_real = tf.image.resize(get_img_center(real_img),[128,128])
        err = tf.reduce_sum(self.rec_loss([big, scenery_real]))
        err += tf.reduce_sum(self.rec_loss([decoded_part, part_real]))
        err += tf.reduce_sum(self.rec_loss([center, center_real]))
    ...
    def train_step(self, real_img):
        ...
        with tf.GradientTape() as discriminator_tape:
            ...
            [real_discriminator_logits,decoded,decoded_part,center] = self.discriminator(real_augmented, training=True,label='real',part=part )
            ...
            reconstruction_loss = self.reconstruction_loss(decoded,decoded_part,part,center, real_img)
