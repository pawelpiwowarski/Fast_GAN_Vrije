...
def conv2D(*args, **kwargs):
    return tfa.layers.SpectralNormalization(L.Conv2D(*args, **kwargs,kernel_initializer='orthogonal',use_bias=False,name='conv2d') )
...
class NoiseInjection(L.Layer):
    def __init__(self, **kwargs):
        ...
        self.weight = self.add_weight(
            'noise', shape=(1), initializer='zeros', 
...
class Discriminator(keras.models.Model):
    ...
    def call(self, x, label='fake', part=None):
        ...
        if label == 'fake':
            ...
            return [self.big_logits(w_h_8), self.decoder(w_h_8), self.part_decoder(cropped)]
